...
namespace FlawBOT.Modules
{
    ...
    public class BotModule : ApplicationCommandModule
    {
        ...
        //{
        //    await ctx.CreateResponseAsync($"Are you sure you want {Program.Settings.Name} to leave the server?").ConfigureAwait(false);
        //    var message = await ctx.CreateResponseAsync(Resources.INFO_RESPOND).ConfigureAwait(false);
        ...
        //public async Task ReportBotIssue(InteractionContext ctx, [Option("report", "Detailed description of the issue.")] string report)
        //{
        //    if (string.IsNullOrWhiteSpace(report) || report.Length < 50)
        //    {
        //        await ctx.CreateResponseAsync(Resources.ERR_REPORT_LENGTH).ConfigureAwait(false);
        //        return;
        //    }

        //    await ctx.CreateResponseAsync(Resources.INFO_REPORT_SENDER).ConfigureAwait(false);
        //    var message = await ctx.CreateResponseAsync(Resources.INFO_RESPOND).ConfigureAwait(false);
        //    var interactivity = await BotServices.GetUserInteractivity(ctx, "yes", 10).ConfigureAwait(false);
        //    if (interactivity.Result is null)
        //    {
        ...
        //        await ctx.CreateResponseAsync("Thank You! Your report has been submitted.").ConfigureAwait(false);
        //    }
        //}
    }
}