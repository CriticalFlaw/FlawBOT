...
namespace FlawBOT.Modules
{
    ...
    public class UserModule : ApplicationCommandModule
    {
        ...
        public async Task GetUser(InteractionContext ctx, [Option("member", "Server user whose information to retrieve.")] DiscordUser member)
        {
            member ??= ctx.Member;
            ...
            var output = new DiscordEmbedBuilder()
                ...
                .AddField("Presence", member.Presence.Status.ToString(), true)
                .WithThumbnail(member.AvatarUrl)
                .WithFooter($"{ctx.Guild.Name} / #{ctx.Channel.Name} / {DateTime.Now}")
                .WithColor(member.BannerColor ?? DiscordColor.Black);
            if (member.IsBot)
        ...
        //public async Task SetUserName(InteractionContext ctx, [Option("member", "Server user name.")] DiscordUser member, [Option("name", "New nickname for the name.")] string name = null)
        //{
        //    member ??= ctx.Member;
        ...
        //        perms = member.PermissionsIn(channel).ToPermissionString();c
        //    var output = new DiscordEmbedBuilder()
        ...
        [SlashRequirePermissions(Permissions.BanMembers)]
        public async Task Unban(InteractionContext ctx, [Option("userId", "Discord user ID to unban from the server.")] long userId, [Option("reason", "Reason for the unban.")] string reason = null)
        {
            var member = await ctx.Client.GetUserAsync((ulong)userId).ConfigureAwait(false);
        ...
        //[SlashCommand("undeafen", "Undeafens a server user.")]
        //[SlashRequirePermissions(Permissions.DeafenMembers)]
        ...
        //public async Task Warn(InteractionContext ctx, [Option("member", "Server user to warn.")] DiscordUser member, [Option("reason", "Warning message.")] string reason = null)
        //{
        //    var output = new DiscordEmbedBuilder()
