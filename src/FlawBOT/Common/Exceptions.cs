    ...
    public class Exceptions
    {
        ...
        public static async Task Process(SlashCommandErrorEventArgs e, EventId eventId)
        {
            switch (e.Exception)
            {
                ...
                case ChecksFailedException cfe:
                    await BotServices.SendResponseAsync(e.Context, $"Command {Formatter.Bold(e.Context.QualifiedName)} could not be executed.", ResponseType.Error).ConfigureAwait(false);
                    foreach (var check in cfe.FailedChecks)
                        switch (check)
                        {
                            case RequirePermissionsAttribute perms:
                ...
                case ArgumentNullException:
                case ArgumentException:
                    await BotServices.SendResponseAsync(e.Context, $"Invalid or missing parameters. For help, use command `.help {e.Context?.QualifiedName}`", ResponseType.Warning);
                    break;
                ...
                case NullReferenceException:
                case InvalidDataException:
                    e.Context.Client.Logger.LogWarning(eventId, e.Exception, $"[{e.Context.Guild.Name} : {e.Context.Channel.Name}] {e.Context.User.Username} executed the command '{e.Context?.QualifiedName ?? "<unknown>"}' but it threw an error: ");
                    await BotServices.SendResponseAsync(e.Context, e.Exception.Message, ResponseType.Error);
                    break;
